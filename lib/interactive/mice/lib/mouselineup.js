...
mouseLineup.prototype.generateMice = function() {
	...
	while (len < nPups){
		console.log("len", len)
		console.log("mouse", this.data[len])
		mice.enter()
			.append('svg:use')
			.attr('class', 'mouse')
			.attr('xlink:href', '#' + this.data[len] + 'mouse')
			.attr('transform', function(d,i){
				output = 'translate(' + [obj.iconProps.width*(0.5*len),0] + ')scale(' + scale + ')';
				return output
			})
		len++
	}
...
mouseLineup.prototype.clearUse = function() {
	if (this.getUseLength()||this.getUseLength() === undefined){
