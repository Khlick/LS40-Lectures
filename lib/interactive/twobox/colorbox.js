var ColorBox = function(opts) {
	...
	this.colorDomain = opts.colorDomain || [0,0];
...
ColorBox.prototype.getGroups = function() {
  ...
  switch (this.colorType){
  	case 'discrete':
  		//color domain disc.
  		var cDom = this.colorDomain.slice();
  		if (cDom.every(v => v==0)) {
  			cDom = [0, this.groups.length - 1, Math.round(d3.max(this.data))];
  		}
  		//colors
		...
		case 'continuous':
			...
			var cDom = this.colorDomain.slice();
			if (cDom.every(v => v==0)) {
				cDom = [d3.min(this.data), d3.max(this.data)];
			}
			//colors
			if (this.groups.length <= 2){
		    var colors = this.colors.slice(0,2);
